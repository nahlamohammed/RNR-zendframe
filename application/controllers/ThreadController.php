...
class ThreadController extends Zend_Controller_Action {
    ...
    public function editAction() {
        ...
        if ($this->_request->isPost()) {
            if ($form->isValid($this->_request->getParams())) {
                $info = $form->getValues();
                $thread_model = new Application_Model_Thread();
                $thread_model->editThread($info);
                $this->redirect("thread/display/id/" . $id . "/toCategoryId/" . $toCategoryId);
            }
        }

        if (!empty($id)) {
            $thread_model = new Application_Model_Thread();
            $th = $thread_model->getThreadById($id);
            $form->populate($th[0]);
        }
        $this->view->form = $form;
        $this->render("add");
    ...
    public function lockthreadAction() {
        ...
        if (!empty($id)) {
            $to_model = new Application_Model_Thread ();
            $thread = $to_model->getThreadById($id);
            $to_model->lockThread($thread);
            $this->redirect("thread/display/id/" . $id . "/toCategory/" . $tocategoryId);
        }
    }
